...
Agent.prototype.insert = function(name, table) {
	...
	var fn = function(db, builder, helper, callback) {
		...
		db.insertOne(data.$set, function(err, response) {
			var id = response ? (response.insertedCount ? response.insertedId : null) : null;
			self.id = id;
			if (!self.isPut)
				self.$id = self.id;
...
Agent.prototype.update = function(name, table) {
	...
	var fn = function(db, builder, helper, callback) {
		...
		if (builder._isfirst) {
			...
			db.updateMany(builder.builder, builder.data, { multi: true }, function(err, response) {
				var data = response ? (response.result.nModified || response.result.n) : 0;
				builder.$callback && builder.$callback(err, data);
				callback(err, data);
			});
...
Agent.prototype.delete = function(name, table) {
	...
	var fn = function(db, builder, helper, callback) {
		...
		if (builder._isfirst) {
			db.removeOne(builder.builder, { single: true }, function(err, response) {
				var data = response ? (response.result.nRemoved || response.result.n) : 0;
				builder.$callback && builder.$callback(data);
				callback(err, data);
			});
		} else {
			db.removeMany(builder.builder, function(err, response) {
				var data = response ? (response.result.nRemoved || response.result.n) : 0;
				builder.$callback && builder.$callback(data);
				callback(err, data);
			});
